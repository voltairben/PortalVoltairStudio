...
import { inviteStudioAdmin } from "@/lib/actions/admin";
import type { InviteAdminResult } from "@/lib/onboarding/invite-admin";

/** Render the modal workflow for inviting a new studio admin. */
export function AdminInviteModal() {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [done, setDone] = useState<InviteAdminResult | null>(null);
  ...
  function onSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await inviteStudioAdmin({
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={close} title={done ? "Admin invited" : "Invite a studio admin"}>
        {done ? (
          <div className="space-y-4">
            <p className="flex items-start gap-2 text-[13px] leading-6 text-ink-muted">
              ...
              Account created and the invite email {done.emailSent ? "sent" : "queued"}. They have
              full studio admin access — every client, project, and deliverable.
            </p>
          ...
          <form action={onSubmit} className="space-y-3.5">
            <Field name="displayName" label="Name" placeholder="Pat Vega" required />
            <Field
              name="email"
              label="Email"
              type="email"
              placeholder="pat@voltairstudio.com"
              required
            />
            <p className="text-[12px] text-ink-subtle">
              They&rsquo;ll get full studio admin access — every client, project, and deliverable.
            </p>
