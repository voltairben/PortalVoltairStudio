/**
 * Branded onboarding email. Pure function, no dependencies — inline HTML only.
 * (React Email / Tailwind v4 aren't a clean fit yet, and email clients need
 * inline styles regardless.)
 *
...
const DEFAULT_LOGIN_URL = `${process.env.NEXT_PUBLIC_APP_URL ?? "https://portalvoltairstudio.com"}/login`;
...
export function renderOnboardingEmail(args: OnboardingEmailArgs): RenderedEmail {
  ...
  const subject = "Your Voltair Studio portal is ready";
